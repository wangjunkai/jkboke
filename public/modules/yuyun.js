...
YUYUN.yuyun = (function () {
    return {
        appConfig:function(){
            //初始化模块的祖先模块
            var applicationModuleName = 'yuyun';
            //初始化模块的初始依赖
            var applicationModuleVendorDependencies = ['ui.router'];
            //初始化模块加载
            var registerModule = function(moduleName, dependencies) {

                YUYUN.yuyun.appConfig.thisApp = moduleName;
                // 创建模块
        ...
        bokeinit: function () {
            return function () {
                if (!YUYUN.yuyun.newindex) {
                    YUYUN.yuyun.newindex = new YUYUN.yuyun.index();
                    YUYUN.yuyun.newindex._initFun();
                }
            }()
        }
    }
